...
function init ()
{
    ...
    var securityDiv = document.getElementById ('security');
    var mode   = modes.REGULAR;
    var activeSubMenu = null;
    var aisTargetTable;
    var aisTargetLayerUpdater;
    var pageButtons = [mapBut, activityBut, securityBut];
    var pageDivs    = [mapDiv, activityDiv, securityDiv];
...
function drawFleet (options)
{
    ...
    fleet = 'fleet' in options ? options.fleet : null;
    show  = 'show' in options ? options.show : true;

    if (fleet)
        fleet.vessels.forEach (drawVessel);
    else
        fleets.enumVessels (drawVessel);
