...
{
    var wndDesc = { position: { top: 0, left: -2, width: Cary.tools.int2pix (parent.clientWidth - 30), height: Cary.tools.int2pix (parent.clientHeight - 18), absolute: true },
                    title: stringTable.waterLevels, parent: parent, visible: true };
    ...
    if ('onClose' in callbacks)
    {
        ...
        wndDesc.onCloseRejected = callbacks.onClose;
    }
...
WaterLevelPane.prototype = Object.create (Cary.ui.Window.prototype);
...
WaterLevelPane.prototype.onInitialize = function ()
{
    ...
    var columns     = [{ title: stringTable.time, width: 170, onItemClick: onSelectTime },
                       { title: stringTable.level, width: 80, onItemClick: onSelectTime },
                       { title: stringTable.battery, width: 50, onItemClick: onSelectTime }];
    var levelList   = new Cary.ui.ListView ({ parent: this.client, columns: columns, visible: true }, { position: 'absolute', top: 140, left: 5, width: 340, height: (this.parent.clientHeight - 180) });
    var fromHourCtl = new Cary.ui.EditBox ({ parent: fromBlock.htmlObject, visible: true, min: 0, max: 24, numeric: true }, hourStyle);
    var fromDateCtl = new Cary.ui.EditBox ({ parent: fromBlock.htmlObject, visible: true, onClick: selectFromDate }, dateStyle);
    var toHourCtl   = new Cary.ui.EditBox ({ parent: toBlock.htmlObject, visible: true, min: 0, max: 24, numeric: true }, hourStyle);
    ...
    function onLoadWaterLevelAreaList (areaList)
    {
        ...
        {
            new Cary.tools.WaitForCondition (function () { return waterLevelAreas !== null; }, 
                                             function ()
                                             {
    ...
    function exportData ()
    {
        ...
        levelList.enumItems (function (item)
                             {
                                 lines.push ([item.itemColumns [0].innerText, item.itemColumns [1].innerText, item.itemColumns [2].innerText]);
